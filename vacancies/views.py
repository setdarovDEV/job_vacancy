...
from .models import JobPost, JobPostRating, PlanChoices
...
class JobPostViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['title', 'description', 'location', 'company__name']  # kengaytirildi ✅
    pagination_class = TenPerPagePagination

    def get_queryset(self):
        # Public ro‘yxatlar uchun by-default budjet ko‘rsatilganlarini ko‘rsatamiz
        base = JobPost.objects.all().order_by("-created_at")
        if self.action in ("list", "recent", "featured"):
            base = base.filter(budget_min__isnull=False, budget_max__isnull=False)
        return base

    def get_serializer_class(self):
        # Public list/featured uchun yengil serializer
        if self.action in ("list", "recent", "featured"):

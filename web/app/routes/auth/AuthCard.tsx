...
/** Inline text link inside auth copy (card footer, consent line). */
export const authLink = "font-medium text-lapis-ink underline-offset-2 hover:underline";
 ...
 * the card itself keeps its solid fallback below md. The view-transition name morphs the card
 * between login, register and password reset instead of a hard cut.
 */
export function AuthCard({ title, subtitle, icon, children, footer }: {
  ...
  return (
    <Card
      ...
      radius="sheet"
      padding="lg"
      className="relative isolate md:border-transparent md:bg-transparent md:shadow-none"
    ...
    >
      ...
      {subtitle && <p className="mt-2 break-words text-md text-ink-2">{subtitle}</p>}
